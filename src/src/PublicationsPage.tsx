...
const PublicationCard = ({
  ...
  return <div ref={ref} className={`transition-all duration-700 ease-out ${inView ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-5 scale-95'}`} style={{
    transitionDelay: `${index * 100}ms`
      ...
      <article className="w-max-7x1 bg-white rounded-2xl border border-solid border-stone-100 overflow-hidden transition-shadow duration-150 hover:shadow-[1px_1px_0px_rgba(241,245,249,1)]">
        <img className="w-full h-52 object-cover rounded-t-2xl" src={publication.image} alt={publication.title} />
